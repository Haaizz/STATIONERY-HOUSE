...
<body>
   ...
   <?php
   //make the query
   $q = "SELECT ID_U, FullName_U, UserName_U, Phone_U, Password_U FROM user ORDER BY ID_U";
   ...
   if($result)
   {
      ...
      while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {
         echo '<tr>
         <td><a href = "edit_customer.php?id='.$row['ID_U'].'"> Edit </a></td>
         <td><a href = "delete_customer.php?id='.$row['ID_U'].'"> Delete </a></td>
         <td>' . $row['ID_U']. '</td>
         <td>' . $row['FullName_U']. '</td>
         <td>' . $row['UserName_U']. '</td>
         <td>' . $row['Phone_U']. '</td>
         <td>' . $row['Password_U']. '</td>
         </tr>';
      }
      //close the table
      echo '</table>';
